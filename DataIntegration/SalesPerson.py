...
#     red = company.query('name=="RED"')[['com_id']]
#     sales = orders.merge(red)[['sales_id']]
#     return sales_person.merge(sales)
...
# Beats
# 74.90%
# https://leetcode.com/problems/sales-person/solutions/3919992/pandas-one-liner-solution-beats-97-detailed-solution-with-each-part-breakthrough/?envType=study-plan-v2&envId=30-days-of-pandas&lang=pythondata
# def sales_person(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
...
# Details
# 692ms
# Beats 7.99%of users with Pandas
# Memory
# Details
# 62.96MB
# Beats 7.03%of users with Pandas
# https://leetcode.com/problems/sales-person/solutions/3903707/easy-to-come-up-with-the-sql-then-you-solve-it-in-pandas/?envType=study-plan-v2&envId=30-days-of-pandas&lang=pythondata
def sales_person(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    df = pd.merge(orders, company, on='com_id')
    red_orders = df[df['name'] == 'RED']
    invalid_ids = red_orders.sales_id.unique()
    return sales_person[~sales_person['sales_id'].isin(invalid_ids)][['name']]
